...
class SslCommerzPaymentController extends Controller
{
    ...
    public function success(Request $request)
    {
        ...
        $order_detials =order::
            ...
            ->select('quantity','product_id','transaction_id', 'status', 'currency', 'amount')->get();

        if ($order_detials[0]->status == 'pending') {
            ...
            if ($validation) {
                ...
                */
                ...
                $update_product = order::where('transaction_id', $tran_id)
                    ...
                    foreach($order_detials as $o){
                        product::where("id", $o->product_id)->update( array(
                            'quantity' => \DB::raw( 'quantity - '.$o->quantity )
                        ) );
                    }
        ...
        } else if ($order_detials[0]->status == 'processing' || $order_detials[0]->status == 'complete') {
            /*
